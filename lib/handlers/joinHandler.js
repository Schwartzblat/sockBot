 ...
 *
 * @param {proto.IWebMessageInfo} notification
...
const procJoin = async (notification, sock) => {
  ...
  //     return;
  // }
  const user = notification.messageStubParameters[0];
  if (user.startsWith('972') && !(await isInBlackList(user.split('@')[0]))) {
    return;
  }
  const chat = await sock.groupMetadata(notification.key.remoteJid);
  if (!isGroupAdmin(sock.user.id.split(':')[0]+'@s.whatsapp.net', chat)) {
    return;
  }
  await sock.groupParticipantsUpdate(notification.key.remoteJid,
      [user], 'remove');
  const date = new Date();
  const dateFormatted = pad(date.getHours())+':'+pad(date.getMinutes())+
      ':'+pad(date.getSeconds())+','+pad(date.getDate())+
      '.'+pad(date.getMonth()+1)+'.'+date.getFullYear();
  console.log('kicked: '+user.split('@')[0]+
      ' from: '+reverse(chat.subject)+' at '+ dateFormatted);
