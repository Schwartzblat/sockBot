...
const customQuoteCommand = require('../../../bin/commands/otherCommands/customQuoteCommand');
const polygraphCommand = require('../../../bin/commands/funCommands/polygraphCommand');
const findBestMatchCommand = require('../../../bin/commands/funCommands/findBestMatchCommand');
const triggeredCommand = require(
    '../../../bin/commands/funCommands/triggeredCommand');
const shadowBanCommand = require('../../../bin/commands/groupCommands/shadowBanCommand');
const listAdminsCommand = require('../../../bin/commands/groupCommands/listAdminsCommand');
const getAdminCommand = require('../../../bin/commands/groupCommands/getAdminCommand');
...
const procCommand = async (message, socket, store, miniGames) => {
  ...
  switch (messageParts[0].substring(1)) {
    ...
    case 'הוסף':
      ...
      break;
    case 'קוד':
      await socket.sendMessage(message.key.remoteJid, {text: 'https://github.com/Schwartzblat/sockBot\nשימו כוכב :)'}, {quoted: message});
      break;
    case 'הסר':
    ...
    case 'פוליגרף':
      ...
      break;
    case 'התאמה':
      await findBestMatchCommand(message, socket);
      break;
    case 'זייף':
      await customQuoteCommand(message, socket);
      break;
    case 'מותרס':
      await triggeredCommand(message, socket);
      break;
    case 'מחק_הכל':
      await shadowBanCommand(message, socket, store);
      break;
    case 'מנהלים':
      await listAdminsCommand(message, socket);
      break;
    case 'מנהל':
      await getAdminCommand(message, socket);
      break;
  }
};
