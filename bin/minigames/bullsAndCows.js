...
class BullsAndCows extends MiniGame {
  ...
  async procMessage(message) {
    ...
    await this.sock.sendMessage(this.chatId, {text: 'ניחשת '+bulls+' בולים ו'+cows+' פגיעות'}, {quoted: message});
  }
  ...
  async gameOver(message=undefined) {
    ...
    await this.sock.sendMessage(this.chatId, {text: constantMessages.gameOver}, {quoted: message});
    await this.#sendMessage('המשחק הסתיים. כדי להתחיל משחק חדש כתבו "!בול_פגיעה".');
  }
}
