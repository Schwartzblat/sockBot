...
const procCommand = async (message, sock) => {
  ...
  try {
    const sql = 'insert into blacklist (name, phone, start, end) values (\'' +
        name + '\', \'' + phone + '\', ' + currentTime + ', ' +
        (currentTime + timeToBlock) + ');';
    db.run(sql);
    await sock.sendMessage(message.key.remoteJid, {text: 'בוצע'},
        {quoted: message});
  } catch (err) {
    console.log(err);
    await sock.sendMessage(message.key.remoteJid, {text: 'בעיה בדאטהבייס'},
        {quoted: message});
  }
};
