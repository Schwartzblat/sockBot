...
let {description, title} = require('../../../config/survey.json')
...
const surveyResults = async (message, sock, store)=>{
    const quotedBase = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
    ...
    const buttonsText = quotedBase.buttonsMessage.buttons.map(button=>button.buttonText.displayText);
    const buttonsId = quotedBase.buttonsMessage.buttons.map(button=>button.buttonId);
    ...
    for(let i=messages.length-1;i>-1;i--){
        ...
        if (!messages[i]?.message?.buttonsResponseMessage || messages[i].message.buttonsResponseMessage.contextInfo.stanzaId !== quoted.key.id){
            continue;
        }

        if(counter.has(messages[i]?.message?.buttonsResponseMessage?.selectedButtonId) && !voted.includes(messages[i].key.participant || messages[i].key.remoteJid)){
            counter.set(messages[i].message.buttonsResponseMessage.selectedButtonId, counter.get(messages[i].message.buttonsResponseMessage.selectedButtonId)+1);
