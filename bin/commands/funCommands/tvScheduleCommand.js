...
const procCommand = async (message, sock) => {
  ...
  switch (removeFirstWord(message.body).split(' ')[0]) {
    case '11':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/1/ערוץ-כאן-11-שידור-חי');
      break;
    case '12':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/12/ערוץ-12-שידור-חי');
      break;
    case '13':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/13/ערוץ-13-שידור-חי');
      break;
    case '41':
    ...
    case '43':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/141/ערוץ-נשיונל-גיאוגרפיק');
      break;
    case '50':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/12564/ערוץ-one-שידור-חי');
      break;
    case '51':
    ...
    case '55':
      url = encodeURI(
          'https://www.isramedia.net/לוח-שידורים/5/ספורט-5-שידור-חי');
      break;
  ...
  const names = data.match(
      new RegExp('<td class="tvguideshowname">.*</td>', 'g')).
      map((item) => item.split('>')[1].split('<')[0].trim());
  const startingTimes = data.match(
      new RegExp('<td class="tvguidetime">.*</td>', 'g')).
      map((item) => item.split('>')[2].split('<')[0].trim());
  const current = data.match(new RegExp(
      // eslint-disable-next-line max-len
      'class="current">\n<td class="tvguidetime"><time datetime=".*>.*<\\/time><\\/td>\n<td class="tvguideshowname">.*<\\/td>',
      'g'))[0].split('\n')[2].split('>')[1].split('<')[0];
