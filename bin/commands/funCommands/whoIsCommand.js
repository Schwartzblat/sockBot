...
const generateAuthorization = async (message, sock) => {
  ...
  fs.writeFile(path.resolve(__dirname, '..\\..\\..\\config\\whoIs.json'),
      JSON.stringify(config), function writeJSON(err) {
        if (err) {
          console.log(err);
        }
      });
...
const procCommand = async (message, sock) => {
  ...
  if (message?.message?.extendedTextMessage?.contextInfo.mentionedJid) {
    phone = message.message.extendedTextMessage.contextInfo.
        mentionedJid[0].split(':')[0].split('@')[0];
  }
  ...
  if (user && user['profile_picture']) {
    await sock.sendMessage(message.key.remoteJid,
        {image: {url: user['profile_picture']}, caption: output},
        {quoted: message});
  } else {
